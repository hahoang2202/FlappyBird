...
def welcomeScreen():
    ...
    while True:
        for event in pygame.event.get():
            ...
            if playbutton.collidepoint(pygame.mouse.get_pos()): #kiểm tra click nút bắt đầu hoặc thoát ra
            
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1: #kiểm tra click trong game
                    mainGame()
...
def mainGame():
    ...
    playerFlapAccv = -8 #Tốc độ
    playerFlapped = False #check tốc độ
    ...
    while True:
        ...
        crashTest = isCollide(playerx, playery, upperPipes, lowerPipes) #Hàm kiểm tra crash
        if crashTest:
            return     
...
def getRandomPipe():
    """
    Tạo vị trí của hai đường ống, một đường ống trên và đường ống dưới khác để hiện thị trên màn
    """
...
if __name__ == "__main__":
    ...
    pygame.init() #Module game
    FPSCLOCK = pygame.time.Clock() #Kiểm soát FPT
    pygame.display.set_caption('Flappy Bird With Python') #Setting caption game
    ...
    while True:
        welcomeScreen() #Hiển thị màn hình ban đầu
        mainGame() #Maingame